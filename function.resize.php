...
function resize($imagePath,$opts=null){
	...
	$defaults = array('crop' => false, 'scale' => 'false', 'thumbnail' => false, 'maxOnly' => false, 
	   ...
	   'cacheFolder' => $cacheFolder, 'remoteFolder' => $remoteFolder, 'quality' => 60, 'cache_http_minutes' => 20);
	...
	if($create == true):
		if(!empty($w) and !empty($h)):
			...
			if(true === $opts['scale']):
				...
				# Changes made to this command to eliminate the canvassed background that always accompanied images. 
				# NOw, they are simply resized WITHIN the given area
				$cmd = $path_to_convert." ". escapeshellarg($imagePath) ." -resize ". escapeshellarg($w ."x". $h) . 
				" -quality ". escapeshellarg($opts['quality'])." ".escapeshellarg($newPath);
			endif;
